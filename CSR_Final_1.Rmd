...
library("RODBC")
```
**Here we are not running the function because we have already saved the scraped data into the CSV files and using it for analysis purpose. If in case one wants to run the functions as well make sure to delete the exisitng csv files**
...
**To ensure no error is there once you have if you have used the function to popluate delete existing CSV files**
...
trending <- function (y) {
y <- gsub(";", ",", y)
find.listr <- list("r,", "?f»","python,", "-3?f»x", "-2?f»7", "python")
